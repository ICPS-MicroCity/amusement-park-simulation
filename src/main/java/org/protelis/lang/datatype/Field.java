...
public interface Field<T> extends Serializable {
    ...
    default Class<? extends T> getExpectedType() {
        // Passare sugli elementi del field e trovare il tipo minimo comune a tutti gli elementi
        /*
        var superclasses = this.valueStream()
                .map(v -> v.getClass())
                .reduce((c1, c2) -> c1.isAssignableFrom(c2) ? c1 : )
                .collect(Collectors.toSet());
        if (superclasses.stream().allMatch(i -> i.equals(superclasses.get(0)))) {
            return (Class<? extends T>) superclasses.get(0);
        }
        return (Class<? extends T>) getLocal().getValue().getClass();
         */
        return (Class<? extends T>) (List.class.isAssignableFrom(getLocalValue().getClass()) ? List.class : getLocalValue().getClass());
    }
